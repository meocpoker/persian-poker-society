...
function getSessionIdFromPath(pathname: string): string | null {
  // expected: /dashboard/sunday/sessions/<uuid>/lock
...
export async function POST(req: Request, ctx: any) {
  ...
  // Redirect back to the session page so the UI updates
  return NextResponse.redirect(new URL(`/dashboard/sunday/sessions/${sessionId}`, req.url));
}